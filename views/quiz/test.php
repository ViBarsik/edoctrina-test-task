...
$this->title = "Quiz '{$Quiz->quiz}'";
$this->registerCssFile('/css/views/quiz/test.css');
$this->registerJsFile('/js/views/quiz/test.js', ['depends' => [ 'yii\web\JqueryAsset' ] ]);
$this->registerJsFile(Yii::$app->params['socket_url'] . '/socket.io/socket.io.js', ['depends' => [ 'yii\web\JqueryAsset' ] ]);
?>
...
<div class="quiz-test text-center">
    ...
    <?=Html::beginForm(['/quiz/test/'.$Quiz->quiz_id], 'post');?>
    <?=Html::submitButton('Save and finish', ['class' => 'btn btn-lg btn-success']);?>
    <?=Html::endForm();?>
